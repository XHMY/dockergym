#!/usr/bin/env python
"""
API example: Interact with ScienceWorld via the DockerGym Web API
...
def single_session_demo(base_url: str, max_steps: int = 20):
    """Run one session on the 'boil' task, picking random valid actions."""
    print("=== Single Session Demo ===\n")

    with Session(base_url, env_id="boil", params={"variation_idx": 0}) as session:
        sid = session["session_id"]
        info = session.get("info", {})
        print(f"Session:  {sid[:8]}...")
        print(f"Task:     {session['env_id']}")
        print(f"Goal:     {info.get('task_description', 'N/A')}")
        ...
        for step in range(1, max_steps + 1):
            action = random.choice(valid_actions) if valid_actions else "look around"
            ...
            valid_actions = step_info.get("valid_actions", [])
            obs_short = result["observation"][:120].replace("\n", " ")

            print(f"  Step {step:2d}: {action}")
            print(f"           -> {obs_short}  [score={score}]")

            if result["done"]:
                print(f"\n  Done! Final score: {score}")
                break
        else:
            print(f"\n  Stopped after {max_steps} steps (score={score})")
...
def main():
    parser = argparse.ArgumentParser(description="ScienceWorld DockerGym API example")
    parser.add_argument(
    ...
    try:
        ...
        print("Start it with: python -m dockergym.envs.scienceworld")
        sys.exit(1)
